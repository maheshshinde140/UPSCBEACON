...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Sub Navbar */}
      <div className="flex text-nowrap font-semibold text-xs justify-between p-2 bg-gray-50 border-b border-gray-300">
        <Link to="/trending" className="mx-2 text-red-600 hover:text-blue-500"><i className="fas fa-chevron-left"></i> Trending Now</Link>
        
        {loading ? (
          <span className="mx-2 text-gray-700">Loading subjects...</span>
        ) : (
          subjects.map((subject) => (
            <div className="relative group" key={subject._id}>
              <Link 
                key={subject._id}
